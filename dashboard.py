...
with tabs[3]:
    ...
    if filtered_df.empty:
        ...
        marker_color = 'lightblue' if mode=="Dark" else 'blue'
        
        # Tambahkan marker dengan loop ringan
        for _, row in filtered_df.iterrows():
            folium.CircleMarker(
                location=[row['lat'], row['lon']],
                radius=5,
                popup=f"Stasiun: {row['station_name']}\nPenyewaan: {row['cnt_day']}",
                color=marker_color,
                fill=True,
                fill_opacity=0.6
...
st.caption("© Bike Sharing Dashboard Modern - Dark/Light Mode ✅")
